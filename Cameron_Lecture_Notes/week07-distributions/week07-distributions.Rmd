...
* One random variable $X \sim N(100, 25)$ takes on the value 75. 

* Another random variable $Y \sim N(10, 7)$, with another normal distribution, takes on the value 24.
...
> Consider sampling $n$ points from any distribution $D$, which has expected value $\mu$ and standard deviation $\sigma$, and then taking the mean of those $n$ points as a random variable $\bar{X}$. Then, $\bar{X} \stackrel{\text{approx}}{\sim} N(\mu, \frac{\sigma}{\sqrt{n}}$).

> This distribution is an approximation, which becomes more accurate as $n$ increases. It will perform poorly at low values of $n$, especially for highly skewed distributions.

* There are three important components to this theorem:
1. The sample mean $\bar{X}$ *approximately* follows a normal distribution, **regardless of the initial distribution $D$**. (!!)
...
3. The more points we sample (increasing $n$), the lower standard deviation we have ($\sqrt{n}$ is in denominator). That is, the mean of 100 random points is likely to be closer to $\mu$ than the mean of 10 random points. (Hence, "Limit").
...
* The sampling distribution is normal again! It will always come out *approximately* normal for *large enough samples* from any initial distribution, there was nothing special about the uniform or normal distributions we showed in examples here.
...
$\bar{X} \sim N(shape * scale, \sqrt{scale} * shape / \sqrt{n})$.

```{r}
pnorm(0.1, xbar_mean, xbar_sd)
```

> **Philosophical Takeaway**: Since CLT dictates $\bar{X}$ approximately has a normal distribution, you can use `pnorm` to calculate probabilities of the sample mean taking on certain values!
