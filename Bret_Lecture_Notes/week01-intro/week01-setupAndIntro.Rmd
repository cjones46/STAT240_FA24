...
* From the [course GitHub](https://github.com/cjones46/STAT240_FA24), navigate to `Bret_Lecture_Notes/week01-intro`.

* **Download** `week01-setupAndIntro.html` into the folder on your computer `STAT240/lecture/week01-intro`.
...
* What just happened there was called **creating a variable**. We'll review this more in depth soon, but we created an object with a name; that name is `x`.
    - Just like "Bret Larget" is a name for a specific person, `x` is now a name for a specific data object.
...
- Bret's lecture files will use the following output type:
...
* To be able to recreate Bret's lecture notes, you'll need to **install** the package `rmdformats`, using `install.packages("rmdformats")`.
