...
$$
\text{SE}(\hat{\beta}_1) = \sqrt{\frac{\sum_{i = 1}^n (y_i - \hat{y}_i)^2/(n-2)}{\sum_{i = 1}^n (x_i - \overline{x})^2}}
= \frac{ \hat{\sigma} }{\sqrt{n} \times \sigma_x}
$$

where:

- $\displaystyle\hat{\sigma} = \sqrt{ \frac{\sum_{i = 1}^n (y_i - \hat{y}_i)^2}{n-2}}$
- $\displaystyle\sigma_x = \sqrt{ \frac{\sum_{i = 1}^n (x_i - \overline{x})^2}{n} }$
...
* $\text{SE}(\hat{\beta}_1)$ in the above output is `0.02667`, the number to the right of our estimate of the slope, $\hat{\beta}_1$ =`-0.22298` (from the coefficients table in the middle of the output).
...
> This rate is also between 1.70 and 2.75 days per decade.
...
$$
(\text{Point Estimate}) \pm (\text{Quantile Confidence Score}) \times (\text{Standard Error})
$$
...
$$
s_{\hat{y}^*} = \hat{\sigma} \times \sqrt{ 1 + \frac{1 + z^2}{n} }
$$

where:

- $\displaystyle \hat{\sigma} = \sqrt{ \frac{\sum_{i=1}^n (y_i - \hat{y}_i)^2}{n-2} }$
- $\displaystyle z = (x^* - \bar{x})/\hat{\sigma}_x$
- $\displaystyle \hat{\sigma}_x = \sqrt{\frac{\sum_{i=1}^n (x_i - \bar{x})^2}{n}}$
...
ggplot(lions, aes(age, black)) +
  geom_point() +
  geom_line(data = as_tibble(intervals),
            mapping = aes(x = 1:13, y = lwr),
            color = "red",
            linetype = "dashed") +
  geom_line(data = as_tibble(intervals),
            mapping = aes(x = 1:13, y = upr),
            color = "red",
            linetype = "dashed") +
  labs(
...
$$
s_{\hat{y}} = \frac{\hat{\sigma}}{\sqrt{n}} \times \sqrt{1 + z^2}
$$
where:

- $\displaystyle \hat{\sigma} = \sqrt{ \frac{\sum_{i=1}^n (y_i - \hat{y}_i)^2}{n-2} }$
- $\displaystyle z = \frac{ x^* - \bar{x}}{\sigma_x}$
- $\displaystyle \sigma_x = \sqrt{ \frac{\sum_{i=1}^n (x_i - \bar{x})^2}{n} }$
