...
> Let the true probability of a chimpanzee picking prosocially with a partner be $p_{\text{partner}}$. Is $p_{\text{partner}}$ greater than 0.5?

> Let the true probability of a chimpanzee picking prosocially with no partner be $p_{\text{no partner}}$. Are $p_{\text{no partner}}$ and $p_{\text{partner}}$ equal?

* We will investigate both of these questions (in order) through confidence intervals and hypothesis testing.

##### Defining Variables

> **Question of interest:** Let the true probability of a chimpanzee picking prosocially with a partner be $p_{\text{partner}}$. Is $p_{\text{partner}}$ greater than 0.5?
...
$$
\hat{p} \stackrel{\text{approx}}\sim \text{N}\Bigg(p, \sqrt{\frac{p(1-p)}{n}}\Bigg)
$$

* This statement relies on the **normal approximation of binomial distributions** concept we learned a few weeks ago - which relies on $np(1-p)$ and not being small. 
...
- With $n = 90$, let's examine the distribution of $\hat{p} = X/n$ when $X \sim \text{Binomial}(n, 60/90)$.
...
$$
\hat{p}_{\text{AC}} = \frac{X + 2}{n +4}
$$

$$
\hat{p}_{\text{AC}} \pm \text{qnorm}(\alpha+(1-\alpha)/2) * \sqrt{\frac{\hat{p}_{\text{AC}}(1-\hat{p}_{\text{AC}})}{n+4}}
$$
...
* For estimating the true $p$, the point estimate is $\hat{p} = X/n$, the sample proportion.
    - When making the **Agresti-Coull adjustment**, your point estimate is $\hat{p}_{\text{AC}} = \frac{X+2}{n+4}$.
...
* The standard error of $\hat{p}$ is based off of $p$ which we don't know.
    ...
    - However, when we make the **Agresti-Coull adjustment** it is $\sqrt{\frac{\hat{p}_{\text{AC}}(1-\hat{p}_{\text{AC}})}{n+4}}$.
...
* In this case, the test statistic comes straight from our model: we know that the observed number of successes $X \sim \text{Binomial}(90, p)$.

* When we assume $p = 0.5$, this turns into $X \sim \text{Binomial}(90, 0.5)$. That is our **null distribution**, which is fully known.
...
* We know that $X \sim \text{Binomial}(200, p)$. Under the null hypothesis which states $p = 0.25$, $X \sim \text{Binomial}(200, p)$.
...
> The sampling distribution of $\hat{p_1} - \hat{p_2}$ is $\text{N}(p_1 - p_2, \text{SE}(\hat{p_1} - \hat{p_2}))$, where the standard error is $\text{SE}(\hat{p}_1 - \hat{p}_2) = \sqrt{ \text{SE}(\hat{p}_1)^2 + \text{SE}(\hat{p}_2)^2 } = \sqrt{ \frac{p_1(1-p_1)}{n_1} + \frac{p_2(1-p_2)}{n_2} }$.
...
> $\hat{p_{\text{1AC}}} = \frac{X_1 + 1}{n_1 + 2}$, and $\hat{p_{\text{2AC}}} = \frac{X_2 + 1}{n_2 + 2}$. These adjustments carry through to the calculations of the point estimate and standard error.

* With the Agresti-Coffe adjustment,

$$
\text{SE}(\hat{p}_{\text{1AC}} - \hat{p}_{\text{2AC}}) = \sqrt{ \frac{\hat{p}_{\text{1AC}}(1-\hat{p}_{\text{1AC}})}{n_1+2} + \frac{\hat{p}_{\text{2AC}}(1-\hat{p}_{\text{2AC}})}{n_2+2} }
$$
...
$$
\hat{p_{\text{1AC}}} - \hat{p_{\text{2AC}}} \pm \text{qnorm(C + (1-C)/2)} * \sqrt{ \frac{\hat{p}_{\text{1AC}}(1-\hat{p}_{\text{1AC}})}{n_1+2} + \frac{\hat{p}_{\text{2AC}}(1-\hat{p}_{\text{2AC}})}{n_2+2} }
$$
...
* We know from the confidence intervals section that $\hat{p_1} - \hat{p_2} \sim \text{N}(p_1 - p_2, \text{SE}(\hat{p_1} - \hat{p_2}))$.

* We can standardize this to:

$$
T = \frac{(\hat{p_1} - \hat{p_2}) - (p_1 - p_2)}{\text{SE}(\hat{p_1} - \hat{p_2})} \sim \text{N}(0, 1)
$$

* Our null hypothesis says that $p_1 - p_2 = 0$, so that term goes away.

* However, $\text{SE}(\hat{p_1} - \hat{p_2}) = \sqrt{ \frac{p_1(1-p_1)}{n_1} + \frac{p_2(1-p_2)}{n_2}}$ is still based on $p_1$ and $p_2$. The null hypothesis doesn't give us specific values, it just says that they are equal.
...
$$
\frac{(\hat{p_1} - \hat{p_2}) - (0)}{\sqrt{ \frac{\bar{p}(1-\bar{p})}{n_1} + \frac{\bar{p}(1-\bar{p})}{n_2}}} \sim \text{N}(0, 1)
$$
