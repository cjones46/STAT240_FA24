...
* This data set contains **one row for every winter season**, which starts in the late months of one year and ends in the early months of the next.
    ...
    + The variable `duration` is the total number of days that Lake Mendota was covered by ice in that winter.
    + The variables `first_freeze` and `last_thaw` contain the dates that the lake first froze and last thawed in that winter.
        + If there is only a single interval, `duration` is the number of days between these two dates.
        + If there are two intervals, `duration` will be smaller than the time interval between the dates as the lake had a thawed period between the first freeze and the last thaw.
        + The variables `ff_june30` and `lt_june30` are a quantitative representation of the `first_freeze` and `last_thaw` variables which count the number of days each of these events follow June 30 of the corresponding first year.
...
> How has the duration of time Lake Mendota is ice covered changed each winter since records have been kept?
...
* The below code takes our 169 winters in `mendota` and reduces it to three rows, one for each century that we want to plot. You'll learn more about this code next week.
